...
class TabContentSectionComponent {
  constructor(section, bus) {
    ...
    this.element = document.getElementById(
      `${TabContentSectionComponent.TAB_CONTENT_ID_PREFIX}-${this.dataId}`
    );
  ...
  getDataId() {
    var _a;
    return (_a = this.section.getAttribute(
      TabContentSectionComponent.TAB_CONTENT_DATA_ID_KEY
    )) !== null && _a !== void 0
      ? _a
      : "00";
  }
...
class TabButtonComponent {
  ...
  getDataId() {
    var _a;
    return (_a = this.button.getAttribute(
      TabButtonComponent.TAB_DATA_ID_KEY
    )) !== null && _a !== void 0
      ? _a
      : "00";
  }
...
class ServiceTabsWidget {
  constructor() {
    ...
    const tabSectionsContainer = document.getElementById(
      ServiceTabsWidget.TAB_CONTENT_SECTIONS_CONTAINER
    );
    ...
    const tabContainer = document.getElementById(
      ServiceTabsWidget.TABS_CONTAINER
    );
    if (tabContainer) {
      const tabs = tabContainer.querySelectorAll(
        ServiceTabsWidget.BUTTON_TAB_CLASS
      );
...
ServiceTabsWidget.TAB_CONTENT_SECTIONS_CONTAINER =
  "del-service-content-sections-container-id";
